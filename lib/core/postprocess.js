...
class PostProcess {
    ...
    async convert() {
        ...
        const completeFile = await this.uniqueFileName(completeDir, capInfo.filename, fileType) + (fileType === "m3u8" ? "" : "." + fileType);
        const capPath = path.join(this.config.recording.captureDirectory, fileType === "m3u8" ? capInfo.filename : capInfo.filename + ".ts");
