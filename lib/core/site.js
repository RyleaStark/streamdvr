...
class Site {
    ...
    async dirSize(directory) {
        ...
        return stats.reduce((accumulator, size) => {
    ...
    async checkFileSize(streamer, file) {
        ...
        const sizeMB = stat.isDirectory() ? Math.round(await this.dirSize(file) / 1048576) : Math.round(stat.size / 1048576);
    ...
    async startCapture(capInfo) {
        ...
        if (capture.pid) {
            const filename = this.dvr.config.recording.autoConvertType === "m3u8" ? capInfo.filename : `${capInfo.filename}.ts`;
    ...
    async endCapture(streamer, capInfo) {
        const fullname = this.dvr.config.recording.autoConvertType === "m3u8" ? capInfo.filename : `${capInfo.filename}.ts`;
        try {
            ...
            if (err.code === "ENOENT") {
                this.print(dvr_1.MSG.ERROR, `${colors.name(streamer.nm)}, ${colors.file(this.dvr.config.recording.autoConvertType === "m3u8" ? capInfo.filename : `${capInfo.filename}.ts`)} not found ` +
